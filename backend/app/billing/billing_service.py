...
class BillingService:
    ...
    def get_capabilities(
        ...
        # FREE LAUNCH
        #
        # Marketplace creation and contact sharing are unrestricted.
        # Wanted Posts are also free and are not scanned during this phase.
        # =========================================================================
        if not self.billing_enabled:
    ...
    def get_billing_info(self) -> MarketplaceBillingInfo:
        ...
        plans = [
            MarketplaceBillingPlan(
                ...
                description=(
                    "Monthly access with your marketplace contact "
                    "available to other marketplace users."
                ),
            ...
            MarketplaceBillingPlan(
                ...
                description=(
                    "Create listings without a listing fee and pay "
                    "only when you connect with another user."
                ),
                connection_fee=connection_fee,
                includes_connection_fee=False,
                contact_access=(
                    "Unlocked after the connection fee is paid."
                ),
            ),
            MarketplaceBillingPlan(
                ...
                description=(
                    "Pay a listing fee for each item you post. Once "
                    "the listing is paid for, its contact is available "
                    "to marketplace users without an additional "
                    "connection fee."
                ),
                listing_fee_per_item=(
                    settings.marketplace_listing_fee_per_item
                ),
                connection_fee=Decimal("0.00"),
                includes_connection_fee=True,
                contact_access=(
                    "Available to everyone in the marketplace after "
        ...
        return MarketplaceBillingInfo(
            ...
            current_mode=(
                "free_launch"
                if not self.billing_enabled
                else settings.marketplace_default_billing_mode
            ),
