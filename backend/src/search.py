...
class RAGSearch:
    def __init__(self, persist_dir: str = "faiss_store", embedding_model: str = "all-MiniLM-L6-v2", llm_model: str = "groq/qwen/qwen3.6-27b"):
        ...
        # Primary model
        primary = ChatLiteLLM(model=llm_model, temperature=0)

        # Fallbacks (any LangChain-compatible model)
        fallback_1 = ChatLiteLLM(model="openai/gpt-4o-mini", temperature=0)
        fallback_2 = ChatLiteLLM(model="groq/llama-3.3-70b-versatile", temperature=0)

        # LangChain's .with_fallbacks() chains them together
        robust_llm = primary.with_fallbacks([fallback_1, fallback_2])

        graph = build_graph(robust_llm)
        self.graph = graph
        print(f"[INFO] Groq LLM initialized: {robust_llm.model_name}. Fallbacks: {[f.model_name for f in robust_llm.fallbacks]}")
    ...
    def search_and_summarize(self, query: str, top_k: int = 5) -> str:
        ...
        # Step 3: Merge both result sets using RRF
        merged_indices = self._rrf_merge(vector_results, bm25_results, top_k=top_k)

        # Step 4: Assemble context from merged top chunks
        texts = [self.chunk_texts[idx] for idx in merged_indices if idx < len(self.chunk_texts)]
        context = "\n\n".join(texts)
        ...
        document = ", ".join(documents)       # convert list → string
